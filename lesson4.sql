...
# Строка 177. 47:56 мин. Решение - одно число. Строка 201 profiles - там birthday
# TIMESTAMPDIFF(YEAR, birthday, NOW) < 12 - Возвращает разницу во времени

SELECT * FROM `profiles`
WHERE TIMESTAMPDIFF(YEAR, birthday, NOW()) < 12; -- подзапрос

SELECT COUNT(*) AS 'Кол-во лайков'
	FROM likes -- строка 188
	WHERE media_id IN (
SELECT user_id FROM `profiles`
WHERE TIMESTAMPDIFF(YEAR, birthday, NOW()) < 12
);

/*SELECT COUNT(*) -- объяснение Юрия
FROM likes l -- заходим в таблицу лайков
JOIN media m ON l.media_id = m.id -- НЕ m.user_id
-- если не таблицы МЕДИА в запросе, то надо из лайков пойти в МЕДИА и сждойнить
JOIN profiles p ON p.user_id = m.user_id 
-- SELECT gender, hometown FROM `profiles`
WHERE TIMESTAMPDIFF(YEAR, birthday, NOW()) < 20; -- подзапрос*/

# 2. Определить кто больше поставил лайков (всего): мужчины или женщины.

SELECT
	CASE gender
		WHEN 'f' THEN 'Жен. полайкали'
		WHEN 'm' THEN 'Муж. полайкали'
		ELSE 'ОЙ!'
	END AS 'Пол', COUNT(*) AS 'Лайки'
FROM `profiles`
JOIN likes ON likes.user_id = `profiles`.user_id 
-- просто JOIN - то же, что INNER JOIN в MySQL
GROUP BY gender;

# 3. Вывести всех пользователей, которые не отправляли сообщения.

SELECT DISTINCT CONCAT(firstname, ' ', lastname, ' (id: ', (id), ')') AS 'Не отправляли сообщений:'
-- DISTINCT - только уникальные значения
-- CONCAT - функция комбинации, объединяет в одну строку
FROM users
WHERE NOT EXISTS (
	SELECT from_user_id
	FROM messages
