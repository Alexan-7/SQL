...
users в таблицу users_old. (использование транзакции с выбором commit 
или rollback – обязательно).
*/
...
DROP PROCEDURE IF EXISTS sp_copy_user;
DELIMITER //
CREATE PROCEDURE sp_copy_user()
BEGIN
	DECLARE `_rollback` BIT DEFAULT b'0'; -- ЧТОБЫ НА ЛЕТУ ПОНИМАТЬ: ROLLBACK или COMMIT
	...
	START TRANSACTION;
		...
		SET @add_user = (SELECT id FROM users WHERE users.id = round(rand()*10));
        DELETE FROM users
        WHERE id = @add_user;
		IF `_rollback` THEN
			ROLLBACK;
...
RETURNS VARCHAR(15) DETERMINISTIC -- функция имеет постоянный результат   или DETERMINISTIC, READS SQL DATA когда есть SELECT
BEGIN
	DECLARE greeting VARCHAR(15); -- объявляем переменные
    ...
    SELECT CASE
        ...
        WHEN CURTIME() > '12:00:00' AND CURTIME() < '18:00:00'
			THEN 'Добрый день!'
        WHEN CURTIME() > '18:00:00' AND CURTIME() < '00:00:00'
			THEN 'Добрый вечер!'
...
CREATE TRIGGER record_creating

CREATE TRIGGER 


/*Ссылки на документацию (оставлю для себя):
