...
CREATE OR REPLACE VIEW v_younger20 AS
SELECT firstname, lastname, gender, hometown -- выбираем из уже объединённой таблицы
FROM users
JOIN `profiles` ON `profiles`.user_id = users.id
WHERE TIMESTAMPDIFF(YEAR, birthday, NOW()) < 20
GROUP BY users.id;

SELECT * FROM v_younger20;
...
SELECT -- моя первая попытка - НЕВЕРНАЯ, но самостоятельная:)
	DENSE_RANK() OVER(ORDER BY COUNT(messages.from_user_id) DESC) AS rank_mes,
	CONCAT(firstname, ' ', lastname) AS 'Отправитель',
    messages.from_user_id AS 'Кол-во его сообщений'
FROM messages -- https://gb.ru/lessons/335986 на 18:45
LEFT JOIN users ON messages.from_user_id = users.id
GROUP BY messages.from_user_id;

SELECT -- вторая попытка - подсмотрел решение и разобрался в ошибках
	DENSE_RANK() OVER(ORDER BY COUNT(messages.from_user_id) DESC) AS rank_mes,
	CONCAT(firstname, ' ', lastname) AS 'Отправитель',
    COUNT(messages.from_user_id) AS 'Кол-во его сообщений'
FROM users -- users сджойнить с messages
...
-- эту задачу полностью решил сам
SELECT id, body, created_at,
LEAD(created_at) OVER(ORDER BY created_at ASC) AS next_message, -- ASC по умолчанию
CONCAT(TIMEDIFF(LEAD(created_at) OVER(ORDER BY created_at ASC), created_at), ' мин.') AS diff_time
FROM messages;